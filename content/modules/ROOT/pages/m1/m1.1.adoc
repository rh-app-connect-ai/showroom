:imagesdir: ../../assets/images
:page-time: 1
include::partial$style.adoc[]

[id="setup-lab3"]
== Setup the Lab working folder

{banner-under-construction}

{empty} +


The following set of instructions prepare the set of files you will be working with:


. Close tabs
+
Before you start this lab, make sure you close in your editor all the tabs (source files) from the previous exercise.
+
image::m3/dev-close-tabs.png[width=100%]
+
{blank}
+
1. Click on the file explorer's `...` button +
1. Ensure your `Open Editors` is ticked [√] +
1. Click the `Close all Editors` button.
+
{empty} +

. Setup your Lab 3 folder
+
[.copypaste]
****
step 19
****
+
[TIP]
--
.Run the command above anytime to reset the exercise.
[%collapsible]
====
The workshop includes handy helper scripts that let you jump to any step or reset the lab to a known good state:

* `step N` — reset the lab to the beginning of step N
* `steps` — list all available steps in the lab
* `start` or `restart` — go to the very beginning (equivalent to `step 1`)
* `ff` — fast-forward to the *next* step
* `rw` — rewind to the *previous* step
====
--
+
{empty} +

. Check your lab folder
+
After executing the commands above, have a look in your editor's tree view to confirm it looks healthy. It should be similar to:
+
--
[.indent2]
📁 workshop +
&nbsp;&nbsp;📁 lab +
&nbsp;&nbsp;&nbsp;&nbsp;📁 query +
pass:[<mark style="padding-left: 2rem; background-color: white; color: grey"><b>&nbsp;⚙&nbsp;&nbsp;</b></mark>] *application.properties* +
pass:[<mark style="padding-left: 2rem; background-color: white; color: purple"><b><i>&nbsp;!&nbsp;&nbsp;</i></b></mark>] *db.camel.yaml* +

[.indent2]
NOTE: Make sure the files are visible in your file explorer in the left panel of _VSCode_.
--
+
{empty} +

. Understand the Camel route
+
The `db.camel.yaml` file contains a simple Camel route designed to query the backend database. The database lives in a secure, air-tight environment and is not reachable yet — but once you enable connectivity in the next exercise, you'll run this route to confirm the connection works:
+
[cols="1a,1a"]
|===
| *Diagram* | *Camel Route (YAML)*

|
[.mermaid]
----
flowchart LR
    T(timer) --> SQL(sql:SELECT) --> L(log)
----

|
[source,yaml]
....
- route:
    id: query-invoices
    from:
      uri: timer:queryInvoices
      parameters:
        repeatCount: 1
      steps:
        - to:
            uri: sql:SELECT count(*) FROM invoices
        - log:
            message: "${body}"
....

|===
+
The route fires once (`repeatCount: 1`), runs a `SELECT count(*)` query against the `invoices` table, and logs the result.
+
{empty} +

[.verification]
****
Do you see the same folder structure and files?

[.success]
You're ready to continue.

[.fail]
Make sure the syntax of the commands are compatible with with your environment and try again.
****


{empty} +
